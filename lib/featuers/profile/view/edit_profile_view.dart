...
import 'package:base_project/featuers/profile/cubit/profile_cubit.dart';
...
import 'package:base_project/featuers/profile/cubit/update_profile_cubit.dart';
...
class _EditProfileViewState extends State<EditProfileView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(AppStrings.editProfile.tr(), style: getBoldStyle(fontSize: 16.sp, color: ColorManager.black)),
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: Stack(
                    children: [
                      CircleAvatar(
                        ...
                        child: CircleAvatar(
                          radius: 60.r,
                          backgroundImage: (_imageUrl != null && _imageUrl!.isNotEmpty ? NetworkImage(_imageUrl!) : (_pickedImageFile?.path??'').isNotEmpty ? FileImage(_pickedImageFile!) : null ) as ImageProvider?,
                          backgroundColor: ColorManager.lightGrey,
                        ),
                      ),
                      if (_isEditMode)
                ...
                DefaultFormField(
                  ...
                  titleStyle: getSemiBoldStyle(fontSize: 15.sp, color: ColorManager.greyTextColor),
                  textStyle: getRegularStyle(fontSize: 15.sp, color: ColorManager.blackText),
                  isUnderLine: true,
                ),
  ...
  child: BlocConsumer<UpdateProfileCubit, BaseState<BaseModel>>(
  listener: (context, state) {
    if (state.status == Status.failure) {
  ...
  builder: (context, state) {
    return DefaultButtonWidget(
                          ...
                          text: _isEditMode ? AppStrings.save.tr() : AppStrings.edit.tr(),
