...
class DefaultExpansionTile extends StatefulWidget {
  ...
  const DefaultExpansionTile({super.key, this.name = '', this.image = '', this.options, this.onTap, this.optionsWidget, this.backgroundColor, this.iconColor, this.textColor, this.radius = 0, this.initiallyExpanded = false, this.borderColor, this.titleTextStyle, this.leadingSvg = '', this.titleWidget});
...
class _DefaultExpansionTileState extends State<DefaultExpansionTile> {
  ...
  Widget _title(){
    return widget.titleWidget??Row(
      children: [
        ...
        Text(
          widget.name,
          style: widget.titleTextStyle??getBoldStyle(fontSize: 15.sp, color: widget.textColor??ColorManager.white,),
        ),
      ],
    );
  }
  ...
  trailingIcon(){
    return Image.asset(ImageAssets.logo,color: ColorManager.primary,height: 15.h,width: 15.w,);
  }
