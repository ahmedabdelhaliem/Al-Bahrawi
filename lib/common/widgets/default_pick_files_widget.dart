...
class DefaultPickFilesWidget<T> extends StatefulWidget {
  ...
  const DefaultPickFilesWidget({
    ...
    this.builder, this.isCircle = false,
  });
...
class _DefaultPickFilesWidgetState extends State<DefaultPickFilesWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return widget.builder != null ? InkWell(
      overlayColor: const WidgetStatePropertyAll(Colors.transparent),
      onTap: () {
        _pickMultipleFiles();
      },
        child: widget.builder!) : widget.isCircle ? _circleImage() :
    Column(
      ...
      children: [
          ...
          child: Row(
            children: [
              DefaultButtonWidget(
                ...
                Flexible(
                  child: Text(
                _files.isEmpty && (widget.remoteFiles??[]).isEmpty ? AppStrings.noFileSelected.tr() : _filesNames.isNotEmpty && widget.isSingle ? _filesNames[0] : '',
        ...
        if(widget.imagesOnly && widget.isSingle && (_files.isNotEmpty || (widget.remoteFiles??[]).isNotEmpty))
          Padding(
            padding: EdgeInsets.only(top: 10.h),
            child: _files.isNotEmpty ? Image.file(
              File(_files.first),
              fit: BoxFit.cover,
        ...
        if ((widget.remoteFiles??[]).isNotEmpty && !widget.isSingle) _remoteFilesWidget(),
    ...
    return InkWell(
      ...
      child: CircleAvatar(
        ...
        child: CircleAvatar(
          ...
          child: _files.isNotEmpty ? null : Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              SvgPicture.asset(IconAssets.camera),
              SizedBox(height: 5.h,),
              Text(AppStrings.uploadImage.tr(),style: getRegularStyle(fontSize: 13.sp, color: ColorManager.greyTextColor),)
            ],
          ),
        ),
      ),
